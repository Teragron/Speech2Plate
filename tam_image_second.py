import random 
filename = f"{random.randrange(1,10)}"
...
with open('okubeni.txt') as f:
    contents = f.read()
    patho = contents
try:
    img = Image.open(f'bura/{patho}/seed_5_00000.png')
except: 
    img = Image.open(f'bura/{patho}/seed_5_00000.jpg')
samples = None
...
subprocess.run(["slicer/slic3r-console", "--load", "my_config_file.ini", f"{filename}.stl", "--scale", "200"])
